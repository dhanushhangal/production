...
for i in `cat expressrunstoprocess` 
do
	python submitForestExpress.py -q cmscaf1nd -o /store/group/phys_heavyions/kjung/ExpressForests/v${1}/ -i ExpressPhysics.$i.${1}.list --proxy=proxyforprod
done

#rm allruns
#cat runstoprocess > allruns
cat expressrunstoprocess >> allruns
