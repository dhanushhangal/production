...
hltCfg = "HIN_EposDigiHLT_cfg.py"
recoCfg = "HIN-EposRECO_cfg.py"
forestCfg = "runForestAOD_pPb_MC_80X.py"
...
for line in f:
    if not line.find('root://cms-xrd-global.cern.ch') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd /afs/cern.ch/user/k/kjung/run2Validation/HLTchecks/CMSSW_8_0_23/src\n')
            scriptFile.write('eval `scram r -sh`\n')
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/%s %s \n' % (workBase,forestCfg,jobsBase))
            scriptFile.write('cp %s/%s %s \n' % (workBase,recoCfg,jobsBase))
	    scriptFile.write('cp %s/%s %s \n' % (workBase,hltCfg,jobsBase))
            scriptFile.write('cp %s/%s .\n' % (workBase,forestCfg))
            scriptFile.write('cp %s/%s .\n' % (workBase,recoCfg))
	    scriptFile.write('cp %s/%s .\n' % (workBase,hltCfg))
#            scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/*.db .\n')
	    scriptFile.write('cmsRun %s outputFile=step2_%d.root maxEvents=-1 inputFiles=%s\n' % (hltCfg,jobCounter,line) )
	    scriptFile.write('cmsRun %s outputFile=step3_%d.root maxEvents=-1 inputFiles=file:step2_%d.root\n' % (recoCfg,jobCounter,jobCounter) )
	    scriptFile.write('rm step2_%d.root\n' % (jobCounter))
